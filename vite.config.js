...
export default defineConfig(({ mode }) => {
    ...
    // Sertifika oluþturma iþlemi yalnýzca yerel geliþtirme ortamýnda yapýlýr
    if (isDev) {
        if (!fs.existsSync(certFilePath) || !fs.existsSync(keyFilePath)) {
            try {
                child_process.execSync(`dotnet dev-certs https --export-path "${certFilePath}" --format Pem --no-password`);
            } catch (error) {
                console.warn("Sertifika oluþturulamadý, ancak development dýþýnda bu sorun deðildir.");
            }
        }
    }
    ...
    return {
        optimizeDeps: {
            include: ['@emotion/react', '@mui/styled-engine'], // Optimize edilen baðýmlýlýklar
        },
        plugins: [
            svgr(),
            plugin(),
        ...
        server: {
            ...
            https: isDev && fs.existsSync(certFilePath) && fs.existsSync(keyFilePath)
                ...
                : false, // Vercel ortamýnda HTTPS devre dýþý býrakýlýr
        },
    };
});
